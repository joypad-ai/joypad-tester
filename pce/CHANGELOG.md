...
- After 30 seconds of inactivity, the screen clears and the 64x64
  joypad logo bitmap bounces across the display, color-cycling
  through red -> green -> yellow -> blue -> magenta -> cyan -> white
  on each wall bounce. Same artwork, palette, and motion cadence
  as the GameCube and GBA testers.
- Logo rendered as four 32x32 hardware sprites in a 2x2 grid (PCE
  4bpp planar format). Sprite data generated by
  `buildtools/make_logo.py` from `pce/branding/logo_64.png` (a
  hand-editable 64x64 1-bit PNG), or rasterised at 64x64 from
  `joypad/branding/logo_solid.svg` via `rsvg-convert` when no
  hand-edit exists.
...
- Source: `pce/src/joypad_tester.c`. Upstream MIT copyright preserved
  in the header.
- HuC tokenizer gotchas documented in the source: comments must be
  pure ASCII (no em-dashes, curly quotes, etc.), and initialiser
  trailing commas are not accepted.

### Release artifacts

- `joypad_tester_v1.0.0.pce`

### Known limitations

- No per-port button labels yet — `joy(N)` state is shown as the
  raw 16-bit hex bit pattern. A labelled per-port readout in the
  style of the GameCube tester is planned for v1.1.0.
